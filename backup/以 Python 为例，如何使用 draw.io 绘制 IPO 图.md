...
与此同时，为避免细粒度的数据包追踪导致图表复杂度过载，本次暂未引入全量数据流图（DFD），而是聚焦于函数调用图（Call Graph / Invocation Diagram）。这种视角能够最直观地暴露函数间的上下游调用拓扑、跨模块调用关系以及潜在的冗余代码，为后续将面向过程脚本重构为高内聚、低耦合的模块化系统奠定基础。
